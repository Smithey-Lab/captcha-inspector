...
$('sandbox-form').addEventListener('submit',async event=>{event.preventDefault();if(busy||starting||token)return;busy=true;starting=true;controls();status('Starting your one-minute browser...');let current='';try{const data=await request({action:'start',target:$('sandbox-target').value.trim(),streamFirst:true});token=data.token;current=data.token;liveUrl=data.liveUrl;endsAt=Date.now()/1000+Math.min(60,Math.max(0,data.remainingSeconds||0));captures=0;nextCapture=0;opening=false;$('sandbox-capture').textContent=`Capture report (${3-captures} left)`;$('sandbox-session').hidden=false;fit();$('sandbox-session').scrollIntoView({block:'start',behavior:'instant'});countdown();clock=setInterval(countdown,1000);busy=false;controls();connect(liveUrl);if(data.needsOpen){viewStatus('Browser starting. The submitted URL opens automatically in about five seconds.');await new Promise(resolve=>setTimeout(resolve,5000));if(current&&current===token&&active())await openSubmitted(true);}else if(data.report)render(data.report);}catch(error){if(!current||current===token)status(error.message,true);}finally{if(!current||current===token){busy=false;starting=false;}controls();}});
