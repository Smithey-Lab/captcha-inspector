...
function analyze(snapshot={}) {
  ...
  const verification=find('verification','Human-verification language',/captcha|verify.{0,40}(?:human|robot)|(?:not|aren.t)\s+a\s+robot|human\s+verification|security\s+(?:check|verification)|(?:browser|chrome|edge).{0,35}(?:repair|fix|update)/i,text,'info','This wording is common on both genuine and fake verification pages.');
  const run=find('run-dialog','Operating-system command prompt instructions',/(?:windows|win|⊞)\s*(?:key\s*)?(?:\+|and)\s*r\b|(?:open|launch|press|use).{0,35}(?:powershell|windows terminal|terminal|run dialog|command prompt)|(?:cmd|powershell)\.exe/i,text,'high','Web verification should not require opening a local command shell or the Windows Run dialog.');
  ...
  const providerEvidence=detectProviders(snapshot);
  const providers=providerEvidence.map(e=>e.provider+' '+(e.source==='script'?'script':e.source==='iframe'?'iframe':'widget marker'));
  ...
  coverage.widgetMarkersChecked=Array.isArray(snapshot.widgetMarkers);
  coverage.frameUrlsChecked=Array.isArray(snapshot.frameUrls);
  if(!text&&!scripts&&!providers.length){verdict='Insufficient page evidence';level='inconclusive';}
  return {schemaVersion:1,level,verdict,findings,providerIndicators:[...new Set(providers)],providerEvidence,coverage,limitations:[
    ...
    'Obfuscated code, uncaptured external scripts, closed shadow roots, image-only instructions, and content inside inaccessible frames can hide behavior. No OCR or external-script execution is performed.',
